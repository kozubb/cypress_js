...
describe('Add products to cart different behaviour', () => {
	it('should add products from listing page and verify cart', () => {
		...
		cart
			.validateProductTitleInBasket(testData.Products.Backpack.Id, testData.Products.Backpack.Name) // Validate backpack title
			.validateProductQuantityInCart(0, '1') // Validate backpack quantity
			.validateProductPriceInCart(0, testData.Products.Backpack.Price, testData.CurrencySymbol) // Validate backpack price
			.validateProductTitleInBasket(testData.Products.BikeLight.Id, testData.Products.BikeLight.Name) // Validate bike light title
			.validateProductQuantityInCart(1, '1') // Validate bike light quantity
			.validateProductPriceInCart(1, testData.Products.BikeLight.Price, testData.CurrencySymbol) // Validate bike light price
	})
	...
	it('should add products from PDP and verify cart', () => {
		...
		cy.url().should('include', `inventory-item.html?id=${testData.Products.Backpack.Id}`)
		productDetails
			.validateProductTitle(testData.Products.Backpack.Name) // Validate title
			.validateProductPrice(testData.Products.Backpack.Price, testData.CurrencySymbol) // Validate price
		...
		cy.url().should('include', `inventory-item.html?id=${testData.Products.BikeLight.Id}`)
		productDetails
			.validateProductTitle(testData.Products.BikeLight.Name) // Validate title
			.validateProductPrice(testData.Products.BikeLight.Price, testData.CurrencySymbol) // Validate price
			.pressAddToCartButton() // Add to cart
	...
	it('should not allow adding the same product twice', () => {
		...
		productListing.validateIfRemoveButtonIsVisible(testData.Products.Backpack.Name)
		...
		productListing.validateIfRemoveButtonIsVisible(testData.Products.BikeLight.Name)
	})
})
